...
def _sanitize_git_origin(dest: Path, full_name: str) -> None:
    """Ensure origin remote URL never persists authentication tokens on disk."""
    git_dir = dest / ".git"
    if git_dir.exists():
        subprocess.run(
            [
...
def import_repository(user_id: str, full_name: str) -> GithubImportResponse:
    ...
    try:
        ...
        _sanitize_git_origin(dest, full_name)
