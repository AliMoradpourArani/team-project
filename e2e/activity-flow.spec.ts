...
test("student signs in and manages only their activity", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Hossein", level: 1 }),
  ).toBeVisible();
  ...
  await expect(
    page.locator(".timeline-item").filter({ hasText: title }),
  ).toHaveCount(0);
...
test("professor sees team overview and a read-only member drill-down", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: /Team overview/i, level: 1 }),
  ).toBeVisible();
  await expect(page.locator(".professor-member-row")).toHaveCount(4);
  await expect(
    page.getByRole("heading", { name: "Project review queue" }),
  ).toBeVisible();

  await page
    .locator(".professor-member-row")
    .filter({ hasText: "Hossein" })
    .getByRole("link", { name: "Hossein" })
    .click();
  await expect(
    page.getByText("Professor · read-only member view"),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Hossein", level: 1 }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Add activity" })).toHaveCount(
    0,
  );
  await expect(page.locator(".item-actions")).toHaveCount(0);
});

test("professor reviews a project and the student sees feedback read-only", async ({
  page,
}) => {
  await signIn(page, "professor", professorPassword);
  await expect(
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("heading", { name: /Start review|Update rubric/ }),
  ).toBeVisible();
  ...
  await page
    .getByLabel("Feedback")
    .fill("E2E professor feedback: approved for submission.");
  ...
  await expect(page.getByText("92/100")).toBeVisible();
  await expect(
    page.getByText("E2E professor feedback: approved for submission."),
  ).toBeVisible();
...
test("student freezes an integrated project and professor sees final preflight blockers", async ({
  page,
}) => {
  await signIn(page, "hossein", studentPassword);
  await page.goto("/projects/team-foundation");
  ...
  await expect(
    page.getByRole("heading", { name: /Not submitted yet|Frozen submission/ }),
  ).toBeVisible();
  await page
    .getByRole("button", {
      name: /Submit frozen snapshot|Submit new frozen version/,
    })
    .click();
  await expect(
    page.getByText(/Submission v\d+ frozen successfully\./),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: /Frozen submission · v\d+/ }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("heading", { name: "Final delivery control" }),
  ).toBeVisible();
  ...
  await expect(page.getByText("Release candidate blocked")).toBeVisible();
  await expect(page.getByText("make delivery-preflight")).toBeVisible();
  await expect(
    page.getByRole("button", { name: "Freeze release candidate" }),
  ).toBeDisabled();
});
