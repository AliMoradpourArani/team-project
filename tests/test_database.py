...
def test_initialize_database_creates_schema_from_scratch(tmp_path):
    ...
    with connect(database_path) as connection:
        ...
        assert row_count(connection, "project_reviews") == 0
...
def test_duplicate_migration_prefix_is_rejected(tmp_path):
    ...
    first = MIGRATIONS_ROOT / "999_first.sql"
    second = MIGRATIONS_ROOT / "999_second.sql"
    ...
    try:
        with pytest.raises(RuntimeError, match="Duplicate migration prefix '999'"):
        ...
        first.unlink()
        second.unlink()
    assert not list(REPOSITORY_ROOT.glob("backend/database/migrations/999_*.sql"))
...
def test_sync_deletes_rows_removed_from_source_files(tmp_path, monkeypatch):
    ...
    (data_root / "projects" / "solo-project.json").write_text(
        '{"id": "solo-project", "owner_id": "solo", "name": "Solo", '
        '"description": "Project", "technology": ["python"], "status": "active"}',
        encoding="utf-8",
    )
    ...
    with connect(database_path) as connection:
        ...
        assert row_count(connection, "projects") == 1

    (user_dir / "2026-01-01.json").unlink()
    (data_root / "projects" / "solo-project.json").unlink()
    with connect(database_path) as connection:
        sync_source_data(connection)
        assert row_count(connection, "activities") == 0
        assert row_count(connection, "projects") == 0
        assert row_count(connection, "users") == 1
